...
def do_crf(gt_prob=0.92, sz=15):
    ...
    paths = []
    for i in range(1, 3):
        paths.append((os.path.join(training_data_stage1_dir, str(i) + '.png'),
                      os.path.join(training_data_stage1_dir, str(i) + '_class.png'),
                      os.path.join(training_data_stage1_dir, str(i) + '_class_crf.png')))
    for i in range(1, 4):
        paths.append((os.path.join(training_data_stage2_dir, str(i) + '.png'),
                      os.path.join(training_data_stage2_dir, str(i) + '_class.png'),
                      os.path.join(training_data_stage2_dir, str(i) + '_class_crf.png')))
    # Python不是真正的多线程，用多线程不会更快，反而会OOM
    for path in paths:
        inference(path[0], path[1], path[2], gt_prob, sz)


def label_to_vis(use_crf):
    print('Visualizing...')
    for i in range(1, 3):
        convert_label_to_vis(os.path.join(training_data_stage1_dir, str(i) + '_class.png'), os.path.join(
            training_data_stage1_dir, str(i) + '_class_vis.png'))
        if use_crf:
            convert_label_to_vis(os.path.join(training_data_stage1_dir, str(i) + '_class_crf.png'), os.path.join(
                training_data_stage1_dir, str(i) + '_class_crf_vis.png'))
    for i in range(1, 4):
        convert_label_to_vis(os.path.join(training_data_stage2_dir, str(i) + '_class.png'), os.path.join(
            training_data_stage2_dir, str(i) + '_class_vis.png'))
        if use_crf:
            convert_label_to_vis(os.path.join(training_data_stage2_dir, str(i) + '_class_crf.png'), os.path.join(
                training_data_stage2_dir, str(i) + '_class_crf_vis.png'))
...
def generate_ccf(use_crf):
    ...
    label_crf_list_1 = [os.path.join(training_data_stage1_dir, '1_class_crf.png'),
                        os.path.join(training_data_stage1_dir, '2_class_crf.png')]
    ...
    print('Generating...')
    if use_crf:
        stat = generate_stat(label_crf_list_1 + label_crf_list_2)
        ...
        if(not os.path.exists(dataset_dir)):
            ...
            print("create dataset s1s2-crf...")
            # {0: 654, 2: 568, 1: 499, 4: 91, 3: 68})
            generate_dataset(dataset_dir, 320, img_list_1 + img_list_2, label_crf_list_1 + label_crf_list_2)
        else:
            print("dataset s1s2-crf exists, pass!")
    else:
