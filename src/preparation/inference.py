...
def inference(fn_im, fn_anno, fn_output, gt_prob=0.92, sz=15):
    ##################################
    ...
    if use_2d:
        ...
        # get unary potentials (neg log probability)
        # 当包含0，只能把0看作一个标签
        # 如果zero_unsure=True且包含0
        # 在unaray_from_labels中一处对整个labels数组-1时会造成0溢出为255
        U = unary_from_labels(labels, n_labels, gt_prob=gt_prob, zero_unsure=not has_zero)
        d.setUnaryEnergy(U)
    ...
    '''
    imsave(fn_output, MAP.reshape(labels.shape))
